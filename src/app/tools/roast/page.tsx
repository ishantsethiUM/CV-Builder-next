...
import { Flame, ChevronLeft, RotateCcw, Sparkles, Copy, Check, AlertCircle, FileText, Upload, X } from "lucide-react";
...
export default function RoastPage() {
  ...
  const roast = async () => {
    ...
    if (!text.trim()) { setError("Could not extract any text from the file. Try a different format."); setExtracting(false); return; }
    setExtracting(false);
    setLoading(true); setResult(null);
    try { setResult(await roastCV(text, level)); }
      ...
      <nav style={{ background: "var(--white)", borderBottom: "1px solid var(--border)", padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60, position: "sticky", top: 0, zIndex: 40, boxShadow: "var(--shadow-sm)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span className="tag" style={{ background: "rgba(201,76,46,.1)", color: "#c84c2e" }}>AI Powered</span>
          </div>
        </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
                  {LEVELS.map(l => (
                    <button key={l.id} onClick={() => setLevel(l.id)}
                      style={{ padding: "15px 14px", borderRadius: 8, textAlign: "left", border: `1.5px solid ${level === l.id ? "#c84c2e" : "var(--border)"}`, background: level === l.id ? "rgba(201,76,46,.05)" : "var(--white)", cursor: "pointer", transition: "all .14s", boxShadow: level === l.id ? "var(--shadow-sm)" : "none" }}>
                ...
                {file ? (
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "18px 20px", borderRadius: 10, border: "1.5px solid #c84c2e", background: "rgba(201,76,46,.04)" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ fontFamily: M, fontSize: 11, color: "var(--muted)", marginTop: 2 }}>{(file.size / 1024).toFixed(0)} KB · {file.name.split(".").pop()?.toUpperCase()}</p>
                    </div>
