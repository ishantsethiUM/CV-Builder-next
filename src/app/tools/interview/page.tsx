...
import { getInterviewQuestions, getInterviewFeedback, extractCvText, token, type InterviewQuestion, type InterviewFeedback } from "@/lib/api";
import { Brain, ChevronLeft, Sparkles, ChevronRight, RotateCcw, Star, Clock, AlertCircle, Lock, CreditCard, Upload, FileText, X, History } from "lucide-react";
...
export default function InterviewPage() {
  ...
  const removeFile = () => { setCvFile(null); setCvText(""); };

  const start = async () => {
    if (credits !== null && credits.toolCredits === 0) { openBuyModal(); return; }
    if (!role.trim()) { setQError("Please enter a target role."); return; }
    setLoadQ(true); setQError("");

    let finalCvText = cvText;

    // Extract text from file if one was uploaded
    if (cvFile && !cvText) {
      setExtracting(true);
      try {
        finalCvText = await extractCvText(cvFile);
      } catch (e: unknown) {
        setQError(e instanceof Error ? e.message : "Could not read the CV file.");
        setLoadQ(false);
        setExtracting(false);
        return;
      }
    ...
    try {
      ...
      setTimer(0);
      setRunning(true);
      deductToolCredit(); // optimistic UI update — backend already deducted
    } catch (e: unknown) {
  ...
  const avgScore = Object.values(feedbacks).length > 0
    ? Math.round(Object.values(feedbacks).reduce((a, b) => a + b.score, 0) / Object.values(feedbacks).length * 10)
    : 0;

  const noCredits = credits !== null && credits.toolCredits === 0;
  const busy = loadQ || extracting;
      ...
      <div className="page-container" style={{ maxWidth: 700 }}>
        ...
        {noCredits ? (
          <div style={{ textAlign: "center", padding: "72px 24px" }}>
            <div style={{ width: 64, height: 64, borderRadius: "50%", background: "rgba(124,92,191,.08)", border: "1.5px solid rgba(124,92,191,.2)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px" }}>
              <Lock size={26} color="#7c5cbf" />
            </div>
            ...
            <p style={{ fontSize: 15, color: "var(--muted)", maxWidth: 380, margin: "0 auto 8px", lineHeight: 1.7 }}>
              You need <strong>1 tool credit</strong> to start an Interview Session.
            </p>
          ...
          <>
            ...
            {step === "setup" && (
              <div className="anim-in">
                ...
                <div className="card-flat" style={{ padding: "26px", display: "flex", flexDirection: "column", gap: 16 }}>
                  {/* Role + Company */}
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                    <div>
                      <label className="label">Target Role *</label>
                      <input className="field" value={role} onChange={e => setRole(e.target.value)} placeholder="e.g. Software Engineer" />
                    </div>
                    <div>
                      <label className="label">Company (optional)</label>
                      <input className="field" value={company} onChange={e => setCompany(e.target.value)} placeholder="e.g. Google, Stripe…" />
                    </div>
                  </div>

                  {/* CV Upload */}
                  <div>
                    <label className="label" style={{ marginBottom: 10 }}>Upload Your CV <span style={{ color: "var(--muted)", fontWeight: 400 }}>(optional — personalises questions)</span></label>
                    <input
                      ref={fileInputRef}
                      type="file"
                      accept={ACCEPT}
                    ...
                    />
                    {cvFile ? (
                      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderRadius: 8, border: "1.5px solid #7c5cbf", background: "rgba(124,92,191,.04)" }}>
                        <div style={{ width: 36, height: 36, borderRadius: 7, background: "rgba(124,92,191,.12)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                          <FileText size={17} color="#7c5cbf" />
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <p style={{ fontFamily: "var(--font-mono)", fontSize: 10.5, color: "var(--muted)", marginTop: 2 }}>{(cvFile.size / 1024).toFixed(0)} KB</p>
                        </div>
                      ...
                      <div
                        ...
                        style={{ padding: "28px 20px", borderRadius: 8, border: `2px dashed ${dragOver ? "#7c5cbf" : "var(--border)"}`, background: dragOver ? "rgba(124,92,191,.04)" : "var(--white)", cursor: "pointer", textAlign: "center", transition: "all .14s" }}
                      >
                        <Upload size={20} color="var(--muted)" style={{ margin: "0 auto 10px" }} />
                        <p style={{ fontSize: 14, fontWeight: 600, color: "var(--ink)", marginBottom: 4 }}>
                          Drop CV here or <span style={{ color: "#7c5cbf", textDecoration: "underline" }}>browse</span>
                        </p>
                        <p style={{ fontFamily: "var(--font-mono)", fontSize: 10.5, color: "var(--muted)" }}>PDF, DOCX, or TXT</p>
                      </div>
                    )}
                    ...
                    {!cvFile && (
                      <div style={{ marginTop: 10 }}>
                        <p style={{ fontSize: 12, color: "var(--muted)", marginBottom: 6 }}>or paste CV text manually:</p>
                        <textarea className="field" rows={3} value={cvText} onChange={e => setCvText(e.target.value)} placeholder="Paste CV text for personalised questions…" style={{ minHeight: 70, fontSize: 13 }} />
                      </div>
                    )}
                  </div>
                  ...
                  {qError && (
                    <div style={{ display: "flex", gap: 8, padding: "10px 14px", background: "rgba(200,76,46,.07)", border: "1px solid rgba(200,76,46,.2)", borderRadius: 6 }}>
                      <AlertCircle size={13} color="#c84c2e" style={{ flexShrink: 0 }} />
                      <p style={{ fontSize: 12.5, color: "#c84c2e" }}>{qError}</p>
                    </div>
                  )}

                  <button onClick={start} disabled={busy} className="btn btn-forest" style={{ justifyContent: "center", padding: "12px", opacity: busy ? .7 : 1 }}>
                    {extracting
                      ? <><div style={{ width: 15, height: 15, border: "2px solid rgba(250,250,245,.3)", borderTopColor: "var(--cream)", borderRadius: "50%" }} className="spin" />Reading CV…</>
                      : loadQ
                        ? <><div style={{ width: 15, height: 15, border: "2px solid rgba(250,250,245,.3)", borderTopColor: "var(--cream)", borderRadius: "50%" }} className="spin" />Generating questions…</>
                        : <><Brain size={14} />Start Session — 1 credit</>}
                  </button>
                </div>
              </div>
            )}
              ...
              <div className="anim-in">
                ...
                {feedbacks[q.id] && (
                  <div className="card-flat" style={{ overflow: "hidden", marginBottom: 13 }}>
                    <div style={{ display: "flex", gap: 11, alignItems: "center", padding: "13px 15px", borderBottom: "1px solid var(--border)" }}>
                      ...
                      <p style={{ fontSize: 13, fontWeight: 600, color: "var(--forest)" }}>AI Feedback</p>
                    </div>
                ...
                <div style={{ display: "flex", gap: 9, justifyContent: "flex-end" }}>
                  <button onClick={() => getFeedback(q.id, q.question, answers[q.id] || "")}
                    disabled={loadFB || !answers[q.id]?.trim() || !!feedbacks[q.id]} className="btn btn-outline btn-sm">
            ...
            {step === "review" && (
              <div className="anim-in" style={{ textAlign: "center" }}>
                ...
                <div style={{ display: "flex", gap: 11, justifyContent: "center", flexWrap: "wrap" }}>
                  <button onClick={() => { setStep("setup"); setQs([]); setAnswers({}); setFeedbacks({}); setCur(0); setTimer(0); setCvFile(null); setCvText(""); }} className="btn btn-outline">
                    <RotateCcw size={13} />New Session
                  </button>
                  <Link href="/tools/history" className="btn btn-outline"><History size={13} />View History</Link>
